...
|--------|------:|--------|-------|
| CONFIG | 11 | All Covered | `scripts/logbook` config + state |
...
| SESSION | 16 | All Covered | `scripts/logbook` session detection; SESSION-5a/5b and SESSION-11a/11b each count as one; SESSION-13 (`notes[]` in session JSON); SESSION-14 (sub-agent token roll-up + `tokens_breakdown`) |
| RETRO | 8 | All Covered | `templates/retro.md`; RETRO-5/5a count as one (Timeline sections plus its `HH:MM` unit label); RETRO-7/8 (`retro` reads `notes[]`, surfaces the count) — `skills/retro/SKILL.md` |
| PUBLISH | 11 | All Covered | `scripts/logbook` retro publish path |
| PRIVACY | 5 | All Covered | Core contract; PRIVACY-5 (notes log is local-only, never published) |
| INSTALL | 14 | All Covered | `commands/logbook.md`, `skills/`, `hooks/`; INSTALL-13/14 (`SessionStart`/`SessionEnd` orphan-notes hook, `hooks/orphan-notes.sh`) |
| COMPLETION | 6 | All Covered | `scripts/logbook` completions + `install-cli` |
| COST | 7 | All Covered | `scripts/logbook` retro estimate-cost; `scripts/model_prices.json`; COST-6/6a count as one (prefix and dotted normalization plus the bracketed context-window suffix) |
| EXPORT | 14 | All Covered | `scripts/logbook` `export`/`import` + archive format (shipped in 0.12.0) |
...
### 2026-08-09 — price refresh, Opus 5 default, bracketed model ids

Refreshed `scripts/model_prices.json` from LiteLLM: four new keys
(`claude-opus-5`, `claude-opus-4-8`, `claude-sonnet-5`, `claude-fable-5`); no
rate changed on an existing model, and the rest of the upstream diff is metadata
`estimate-cost` doesn't read.

- **Spec (SPEC.md)** — `[COST-3]` default model `claude-opus-4-7` →
  `claude-opus-5`; `[COST-4]` recognized-id floor updated to lead with the
  current generation; `[COST-6a]` added for the bracketed context-window suffix.
- **Implementation (`scripts/logbook`)** — `DEFAULT_MODEL` bumped;
  `_resolve_model` strips a trailing `[…]` so `claude-opus-5[1m]` — the exact
  model id Claude Code reports for a long-context session — resolves instead of
  exiting `unknown model`. (The transcript's own `model` field, which
  `logbook session` reports, carries the base id.)
- **Coverage delta** — none; still 129, COST-6/6a count as one.

### 2026-08-09 — Coverage refresh (spec-status)

Added `[RETRO-5a]`: the retro template's Timeline column names its unit as
elapsed wall-clock `HH:MM`, so a generator computing a duration can't render it
as `MM:SS` (indistinguishable under an hour, nonsense above it). Coverage
unchanged at 129 — RETRO-5/5a count as one per this ledger's lettered-ID
convention.
...
- **Spec (SPEC.md)** — `[NOTE-22]` (`note harvest <sid>` moves the log to
  ...
  (a retro harvests at publish time). `[INSTALL-13]`/`[INSTALL-14]` (the
  `SessionStart`/`SessionEnd` orphan-notes hook). `[CLI-12]` and `[PRIVACY-5]`
...
- **Coverage delta** — +6 normative requirements (NOTE-22..25, INSTALL-13/14);
  123 → 129, all Covered.

### 2026-07-08 — sub-agent token roll-up + EXPORT backfill

Added `[SESSION-14]`: `logbook session` now aggregates token usage across the parent
...
- **Spec (SPEC.md)** — `[SESSION-14]` (roll-up + `tokens_breakdown` shape).
...
- **Backfill** — reconciled the coverage table with the `EXPORT-1..14` requirements
  shipped in 0.12.0, whose export/import surface landed without a STATUS update.
- **Coverage delta** — 108 → **123** (+1 SESSION-14, +14 EXPORT backfill). SESSION 15 → 16,
  new EXPORT row (14).
...
- **Spec (SPEC.md) → v0.2** — `NOTE` concern rewritten. The `New Session`
  ...
  `[NOTE-20]` (`note list`), `[NOTE-21]` (schema-is-contract). `[SESSION-13]`
  (`notes[]` in session JSON), `[CLI-12]` (`note add`/`list`), `[PRIVACY-5]` (log is
...
- **Coverage delta** — 97 → **108** normative requirements (+11). NOTE 15 → 21,
  CLI 11 → 12, SESSION 14 → 15, RETRO 6 → 8, PRIVACY 4 → 5.
...
all disagreed); SESSION row 12 → 14 (lettered splits each count as one); the "+7
...
- **Spec edit (SPEC.md)** — `[SESSION-8]` now requires `concurrency` in the
...
- **Coverage delta** — none; `[SESSION-8]` is an existing requirement with expanded
  scope. Still 75 / 75.
...
- **Spec edits applied (SPEC.md)**
  ...
  - `[SESSION-2]`/`[SESSION-3]`: split the key-set guarantee from the
    name-definition note and removed the duplicated `slug` definition that
    overlapped `[SESSION-3]`. SESSION-2 now states the required keys plus the `name`
    definition; SESSION-3 defines `slug` as the slugified form of `name`.
  - `[SESSION-5]` → `[SESSION-5a]`/`[SESSION-5b]`: decomposed the 3-level detection +
    fallback into additive lettered sub-requirements (PID-lookup hit;
    project-directory fallback).
  - `[SESSION-11]` → `[SESSION-11a]`/`[SESSION-11b]`: same decomposition for `session-id`
    (detection precedence; error path on no-session).
  - `[SESSION-12]`: reworded from descriptive prose into Optional EARS form
    ("Where the platform is not macOS, the behavior of [SESSION-6]/[SESSION-7] is
    undefined").
  - Left `CONFIG-4`, `CONFIG-5`, `RETRO-4`, `RETRO-5`, `INSTALL-2` as-is — acceptable
    Ubiquitous "shall contain/include" form.

- **Coverage delta**
  - No net change: 74 / 74 normative requirements, now all Covered (CLI-2
    moves Partial → Covered). The SESSION-5/SESSION-11 lettered splits re-express
...
### 2026-05-17 — `note` skill spec'd, INSTALL-4 relaxed
...
- **Coverage delta**
  ...
  - All sampled prior requirements (CONFIG-7, CONFIG-11, CLI-10, SESSION-4/5/11, PUBLISH-5..10, INSTALL-12, COST-3/4) remain covered. The only commit since the previous audit was additive.
...
- **Spec edits applied (SPEC.md)**
  - `[INSTALL-3]` corrected: `/logbook <args>` → `/logbook:logbook <args>` (the
    actual plugin-namespaced slash-command form).
  - `[INSTALL-4]` broadened to "one or more skills under `skills/`" and now
    enumerates both `retro/` and `session-id/`.
  - `[SESSION-2]` added `name` to the enumerated session-JSON keys.
  - `[SESSION-8]` added `compaction` and `git_activity` as optional `detailed`
    sub-blocks (these have shipped since 0.3.x but were undocumented).
  - `[CLI-10]` added — covers `--version` / `-v` (shipped in 0.3.0).
  - `[INSTALL-11]` added — covers `install-cli [--dir]` semantics.
  - `[INSTALL-12]` added — covers the headline 0.4.0 `SessionStart`
    cli-freshness hook.
  - `[SESSION-12]` added — declares Copilot/Cursor detection as macOS-only.
...
- **Backlog (advisory; not addressed this pass)**
  - EARS conformance polish for definitional / static-property requirements
    (CONFIG-4, CONFIG-5, RETRO-4, RETRO-5, RETRO-6, INSTALL-1, INSTALL-10). These read
